...
pub fn add_task(journal_path: PathBuf,task:Task) -> Result<()>{
    ...
    if Path::new(&journal_path).exists(){
        file = OpenOptions::new().read(true).write(true).open(journal_path)?;
    }
    else{
        
        file = OpenOptions::new().create_new(true).read(true).write(true).open(journal_path)?;
    }
...
pub fn list_tasks(journal_path: PathBuf) -> Result<()> { 
    ...
    //parse the file and collect the task
